...
def cut_per_thr(img, output, start_x, start_y):
    ...
    while y + piece_size <= img.shape[1]:
        ...
        # path = "/tmp/tmptxt/{}.txt".format(dst, start_y)
        # lock.acquire()
        # with open(path, 'a+') as f:
        #     f.write(str(y + start_y) + '\n')
        # lock.release()
        ...
        #     x = x + int(piece_size * overlap)
        # y = y + int(piece_size * overlap)
        x = 0
        # import time
        # time.sleep(3)
...
def get_multi_thr_inf(w, h):
    '''
    :param w: kfb width
    :param h: kfb height
    :return: a tuple, (x, y)
            x is piece number of rows
            y is a list, each element is a tuple, (start_y, width)
    '''
    piece = 1
    col_per_thr = w // num_thr

    while col_per_thr * h > limit_size:
        h //= 2
        piece *= 2
    y_list = []
    start_y = 0
    ...
    for thr_idx in range(num_thr):
        ...
        # print("this width is ", next_start_y + overlap * piece_size - start_y)

        y_list.append((int(start_y), int(next_start_y + overlap * piece_size - start_y)))
...
if __name__ == '__main__':
    ...
    for num in range(len(src_dst)):
        ...
        print("per row and col", image_per_piece_col,
              image_per_piece_row, "image cnt is -->", image_count)
        ...
        for read_thr_idx in range(num_thr):
            ...
            read_y = w_per_col * read_thr_idx

            col_width = w_per_col
            if read_thr_idx != 0:
                read_y -= int(image_size * overlap)
                col_width = w_per_col + int(image_size * overlap)

            proc = Process(target=cut_one_image,
                           args=(kfb_name, dst, 0, y_list[read_thr_idx][0], y_list[read_thr_idx][1], h, read_thr_idx))
        ...
        # image_count = int((image_per_piece_row+1) * num_thr_cut * (num_thr-1)
        #                   * image_per_piece_col * pieces
        #                   + image_per_piece_row * num_thr_cut
        #                   * image_per_piece_col * pieces
        #                   )
